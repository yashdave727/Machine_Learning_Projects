...
# plot the data points in X according to cluster membership, R.
# That is, colour the datapoints red, green or blue, according to the responsibilities, R.
# assume 3 clusters.
def plot_clusters(X,R):
    # sort columns by total responsibility (ie, cluster size)
    Rsum = np.sum(R,axis=0)
    idx1 = np.argsort(Rsum)    # order = smallest, middle, largest
    idx2 = [idx1[0],idx1[2],idx1[1]]    # rgb = smallest,largest,middle
    Colours = R[:,idx2]
    plt.scatter(X[:, 0], X[:, 1], c=Colours, s=2)
